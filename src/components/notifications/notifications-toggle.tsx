...
export function NotificationsToggle() {
  ...
  const [subscribedState, setSubscribedState] = useState<boolean | null>(null);
  const [toggling, setToggling] = useState(false);
  const [sending, setSending] = useState(false);

  const subscribed = user ? subscribedState : null;
  ...
  useEffect(() => {
    ...
    notifications
      .isSubscribed()
      .then((r) => {
        if (!cancelled) setSubscribedState(r.subscribed);
      })
      .catch(() => {
        if (!cancelled) setSubscribedState(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  async function handleToggle() {
    ...
    setSubscribedState(wantOn); // optimistic
    try {
      const result = wantOn
        ? await notifications.subscribe()
        : await notifications.unsubscribe();
      setSubscribedState(result.subscribed);
      if (!isMobileHost) {
      ...
      setSubscribedState(!wantOn); // revert
